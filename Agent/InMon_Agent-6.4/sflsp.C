...
*/
  ...
  static void setDefaults(SflSp *sp)
  {
    ...
    //sp->counterSamplingInterval = interval;
    sp->promiscuous = 0;
    sp->timeout_ms = 100;
    sp->batch = 100;
    sp->snaplen = 128;
...
-t counterSamplingInterval:  counter sampling interval\n\
  ...
  */
  ...
  {
    int in;
    while ((in = getopt(argc, argv, "d:i:A:a:C:c:s:t:PvTS:")) != -1) {
      switch(in) {
      ...
      case 't': sp->counterSamplingInterval = atoi(optarg); break;
